				...
				<td>
					...
					<select name="u_phone">
						<c:forEach var="first" items="${firstList }">
							<option value="${first }" <c:if test="${fn:split(loginInfo.u_phone, '-')[0] eq first }">selected</c:if>>${first }</option>
						</c:forEach>
					</select>
					<input type="text" name="u_phone" value="${fn:split(loginInfo.u_phone, '-')[1]}" maxlength="4" required> - 
					<input type="text" name="u_phone" value="${fn:split(loginInfo.u_phone, '-')[2]}" maxlength="4" required>
				</td>
			</tr>
			<tr>
				<th>주소</th>
				...
				<td>
					...
					<input type="text" name="u_address" value="${loginInfo.u_address eq '-' ? '': fn:split(loginInfo.u_address, ',')[2]}" id="sample4_detailAddress" placeholder="상세주소">
				</td>
			</tr>
			<!-- <tr>
				<th>* 결제수단</th>
				...
				<td>
					...
					<select name="u_color">
						<c:forEach var="pc" items="${pcList }">
							<option value="${pc }" <c:if test="${pc eq loginInfo.u_color }">selected</c:if>>${pc }</option>
						</c:forEach>
					</select>
				</td>
			</tr>
